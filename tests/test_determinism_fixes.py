...
from lux_pipeline.process.merger import MergeHandler, RecordMerger
...
class StubIndex(dict):
    """Mimics the LmdbIndex wrapper: reconcile() checks .index for an open
    handle and calls .open() otherwise."""
...
def make_lmdb_reconciler(name_index=None, id_index=None):
    ...
    r.debug = False
    r.debug_graph = {}
    r.name_index = StubIndex(name_index or {})
    r.id_index = StubIndex(id_index or {})
    return r
...
def test_aat_extract_names_lowercases_and_keeps_best_priority():
    from lux_pipeline.sources.getty.reconciler import AatReconciler
...
def test_iter_done_refs_skips_blank_lines(tmp_path, monkeypatch):
    ...
    rm.configs = types.SimpleNamespace(temp_dir=str(tmp_path))
