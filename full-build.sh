...
#
echo "Starting Reconciliation Phase for IPCH"
...
do
    echo $count
    nohup python ./run-reconcile.py $count 24 --ipch > ../data/logs/reconcile_$count.txt 2>&1 &
done
...
do
    echo $count
    nohup python ./run-merge.py $count 24 --ipch > ../data/logs/merge_$count.txt 2>&1 &
done
