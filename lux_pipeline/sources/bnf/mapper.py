...
class BnfXmlMapper(Mapper):
    ...
    def guess_type(self, rec, ident):
        ...
        try:
            classes = rec.xpath(
                f"//rdf:Description[@rdf:about='https://data.bnf.fr/ark:/12148/{ident}#about']", namespaces=nss
            )[0]
        except:
            classes = None
    ...
    def handle_group(self, rec, ident, top):
        ...
        dof = me.xpath(
            './bnf-onto:firstYear[@rdf:datatype="http://www.w3.org/2001/XMLSchema#integer"]/text()', namespaces=nss
        )
        if len(dof) >= 1:
            dof = self.to_plain_string(dof[0])
        dod = me.xpath(
            './bnf-onto:lastYear[@rdf:datatype="http://www.w3.org/2001/XMLSchema#integer"]/text()', namespaces=nss
        )
        if len(dod) >= 1:
            dod = self.to_plain_string(dod[0])
        if dof:
