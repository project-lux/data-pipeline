...
class MlMapper(Mapper):
    ...
    def __init__(self, config):
        ...
        self.ignore_props = ['identified_by', 'referred_to_by', 'equivalent', 'access_point', 'dimension', \
            'subject_of', 'attributed_by', 'used_specific_object', 'contact_point', 'defined_by', \
            'shown_by', 'carried_by', 'digitally_carried_by', 'digitally_shown_by', \
            'approximated_by', 'language', 'technique', 'occurs_during', \
    ...
    def transform(self, record, rectype=None, reference=False):
        ...
        if 'identified_by' in data:
            for i in data['identified_by']:
                if i['type'] == 'Identifier' and 'content' in i and \
                    i['content'].startswith('ils:yul:mfhd:'):
                    isMFHD = True
                    break
                ...
                if 'digitally_carried_by' in s:
                    for do in s['digitally_carried_by']:
                        if 'access_point' in do:
                            ...
                            if ap.startswith("https://collections.britishart.") or \
                                ap.startswith("https://artgallery.yale") or \
                                ap.startswith("https://collections.peabody") or \
                                ap.startswith("https://archives.yale.edu") or \
                                ap.startswith("https://search.library.yale"):
        ...
        if pfx == "agent":
            facets['nationalityId'] = [x['id'] for x in data.get('classified_as', []) if self.globals['nationality'] in 
            ...
            facets['occupationId'] = [x['id'] for x in data.get('classified_as', []) if self.globals['occupation'] in 
                [y['id'] for y in x.get('classified_as', [])] and 'id' in x]
            ...
            if 'carried_out' in data:
                for co in data['carried_out']:
                    if self.globals['active'] in [x['id'] for x in co.get('classified_as', []) if 'id' in x] and 'took_place_at' in co:
                        facets['agentActivePlaceId'].extend([y['id'] for y in co['took_place_at'] if 'id' in y])

        if data['type'] == 'Person':
            facets['personId'] = data['id']
            # Do person indexes here
            facets['genderId'] = [x['id'] for x in data.get('classified_as', []) if self.globals['gender'] in 
                [y['id'] for y in x.get('classified_as', [])] and 'id' in x]            
            ...
            facets['superType'] = [x['id'] for x in data.get('classified_as', []) if self.globals['typeOfWork'] in 
                [y['id'] for y in x.get('classified_as', [])] and 'id' in x]
        elif data['type'] == 'Set':
            if 'used_for' in data:
        ...
        for r in reffed:
            ...
            elif data['type'] in ['Group', 'Person', 'Place', 'Activity', 'Period', 'Type', 'Language', 'MeasurementUnit']:
                        ...
                        for dcb in so['digitally_carried_by']:                 
                            if 'access_point' in dcb:
                                for apo in dcb['access_point']:
                                    if 'id' in apo:
                                        ap = apo.get('id', '')
                                        if ap and not ap.startswith('https://search.library.yale.edu/') \
