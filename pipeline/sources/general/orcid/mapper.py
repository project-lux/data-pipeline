...
class OrcidMapper(Mapper):
    ...
    def transform(self, record, rectype, reference=False):
        ...
        if not reference:
            ...
            # URLs
            for rurl in rec.xpath('./person:person/researcher-url:researcher-urls/researcher-url:researcher-url', namespaces=self.nss):
            ...
            for ext in rec.xpath('./person:person/external-identifier:external-identifiers/external-identifier:external-identifier', namespaces=self.nss):
            ...
            for dist in acts.xpath('./activities:distinctions/activities:affiliation-group/distinction:distinction-summary', namespaces=self.nss):
            ...
            for empl in acts.xpath('./activities:employments/activities:affiliation-group/employment:employment-summary', namespaces=self.nss):
                ...
                orgId = self.get_text(empl, './common:organization/common:disambiguated-organization/common:disambiguated-organization-identifier')
                if orgId and orgName:
                    top.member_of = model.Group(ident=f"https://ringgold.com/{orgId}", label=orgName)

            # Membership: member_of
            for memb in acts.xpath('./activities:memberships/activities:affiliation-group/membership:membership-summary', namespaces=self.nss):
                orgName = self.get_text(memb, './common:organization/common:name')
                orgId = self.get_text(memb, './common:organization/common:disambiguated-organization/common:disambiguated-organization-identifier')
                if orgId and orgName:
                    top.member_of = model.Group(ident=f"https://ringgold.com/{orgId}", label=orgName)
