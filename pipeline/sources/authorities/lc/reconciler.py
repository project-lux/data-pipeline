...
from pipeline.process.base.reconciler import SqliteReconciler, LmdbReconciler
...
class LcshReconciler(LmdbReconciler):
    ...
    def should_reconcile(self, rec, reconcileType="all"):
        ...
        if not rec['type'] in ['Type', 'Material', 'Currency', 'Language', 'MeasurementUnit', 'Place']:
            return False
        elif 'equivalent' in rec:
            # Already reconciled
            eqids = [x['id'] for x in rec.get('equivalent', []) if \
                x['id'].startswith('http://id.loc.gov/authorities/')]
            return not eqids
        else:
            return True
