...
class MergeHandler(object):
    ...
    def pre_merge_fixes(self, record, to_do):
        ...
        if record["data"]["type"] == "Place":
            # does any external have part_of?
            trash = False
            for s, r in to_do:
                if s["type"] == "external" and "part_of" in r["data"]:
                    trash = True
                    break
            if trash:
                    ...
                    for i in range(len(mc)):
                        if mc[i] != rc[i]:
                            if not "id" in mc[i] or not "id" in rc[i]:
                                ...
                                mc[i].get("_label", "_A") == rc[i].get("_label", "_B")
    ...
    def merge_common(self, rec, merge, msource="", myskip=None):
        ...
        if "identified_by" in merge and "identified_by" not in skip:
            ...
            nm_conts = {
                ...
                for x in rec["identified_by"]
                if x["type"] == "Name" and "content" in x and x["content"]
            }
            id_conts = {
                str(x["content"]).strip().lower(): x
                for x in rec["identified_by"]
                if x["type"] == "Identifier" and "content" in x and x["content"]
            }
                    ...
                    if "classified_as" in i:
                        ...
                        for icxn in i["classified_as"]:
                            ...
                            if ic == primaryName:
                                ...
                                if primaryName not in mcxns and alternateName not in mcxns:
            ...
            if conts:
                ...
                for i in merge["referred_to_by"]:
                    if "content" in i:
                        ...
                        cont = i["content"].strip()
                        if not cont in conts:
                            if has_ai and "AI generated" in cont:
                                # don't add multiple
                                pass
                            else:
            ...
            for s2 in merge["subject_of"]:
                if "digitally_carried_by" in s2:
                    for do in s2["digitally_carried_by"]:
                        if (
                            "access_point" in do
                            and "id" in do["access_point"][0]
                            and not do["access_point"][0]["id"] in rec_aps
                        ):
