...
class Mapper(object):
    ...
    def __init__(self, config):
        ...
        self.lang_three_to_two = {
            'por':'pt', 'deu':'de', 'ger':'de', 'eng':'en', 'fra':'fr', 'fre':'fr',
            'spa':'es', 'zho':'zh', 'chi':'zh', 'hin':'hi', 'afr':'af', 'alb':'sq',
            'sqi':'sq', 'ara':'ar', 'bul':'bg', 'bos':'bs', 'cat':'ca', 'ben':'bn',
            'rus':'ru', 'nld':'nl', 'dut':'nl', 'fin':'fi', 'ile':'is', 'gle':'ga',
            'ita':'it', 'fas':'fa', 'per':'fa', 'guj':'gu', 'kor':'ko', 'lat':'la',
            'lit':'lt', 'mac':'mk', 'mkd':'mk', 'jpn':'ja', 'hrv':'hr', 'ces':'cs',
            'cze':'cs', 'dan':'da', 'ell':'el', 'gre':'el', 'kat':'ka', 'geo':'ka',
            'heb':'he', 'hun':'hu', 'nor':'no', 'pol':'pl', 'ron':'ro', 'rum':'ro',
            'slk':'sk', 'slo':'sk', 'slv':'sl', 'srp':'sr', 'swe':'sv', 'tur':'tr',
            'cym':'cy', 'wel':'cy', 'urd':'ur', 'swa':'sw', 'ind':'id', 'tel':'te',
            'tam':'ta', 'tha':'th', 'mar':'mr', 'pan':'pa'}
    ...
    def post_mapping(self, record, xformtype=None):       
        ...
        if record is None and self.debug: print(f"fix_links killed record")
        record = self.break_cycles(record, xformtype)
        if record is None and self.debug: print(f"break_cycles killed record")
        record = self.trash_different_from(record)
        if record is None and self.debug: print(f"trash_different killed record")
        return record
